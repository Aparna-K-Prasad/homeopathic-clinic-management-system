...
if ($conn->connect_error) {
    $alertMessage = showAlert('error', 'Connection Failed', 'Database connection failed', 'appt.html');
} else {
    ...
    if (!$loggedInUsername) {
        ...
        if ($result->num_rows === 0) {
            $alertMessage = showAlert('error', 'User Not Found', 'Please register first', 'reg.php');
        } else {
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                if (!isset($_POST['appt_date']) || !isset($_POST['appt_time'])) {
                    $alertMessage = showAlert('warning', 'Missing Information', 'Please select both date and time', 'appt.php');
                } else {
                    ...
                    if ($checkRow['count'] > 0) {
                        $alertMessage = showAlert('warning', 'Time Slot Unavailable', 'This time slot is already booked', 'appt.php');
                    } else {
                        // Check token limit
                        ...
                        if ($tokenRow['count'] >= 12) {
                            $alertMessage = showAlert('error', 'Token Limit Reached', 'Maximum appointments reached for this date', 'appt.php');
                        } else {
                            // Insert appointment
                            ...
                            if ($insertStmt->execute()) {
                                $alertMessage = showAlert('success', 'Appointment Request! ', 'Your appointment request has been sent.We will get back to you shortly.', 'appt.php');
                            } else {
                                $alertMessage = showAlert('error', 'Booking Failed', 'Failed to book appointment. Please try again.', 'appt.php');
                            }
                            $insertStmt->close();
                        }
                        $tokenStmt->close();
...
<head>
    ...
    <link rel="stylesheet" href="/minipro/css/appt.css">
...
<body> 
    ...
    <header>
        <div class="header-container">
            ...
            <nav>
                <ul class="nav">
                    <li class="nav-item"><a class="nav-link" href='/minipro/home/home.html' style="font-size: 30px;">Home</a></li>
                    <li class="nav-item"><a class="nav-link" href='/minipro/signlog/logout.php' style="font-size: 30px;">Log Out</a></li>
